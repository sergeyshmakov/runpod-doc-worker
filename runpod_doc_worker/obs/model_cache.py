"""Finding a model in the Hugging Face cache, and reporting what was found.

The hard part is not locating a directory. It is answering honestly when the answer
is partial: a snapshot may be a symlink into a blob store, a download may have
stopped halfway, and a path may resolve outside the cache it appears to sit in.
Each of those is reported as what it is rather than flattened into "found" or
"missing", because a probe that rounds a partial download up to "ready" sends a
worker into a job it cannot finish.
"""
...
from runpod_doc_worker.obs import dirwalk
...
def find_model_dir() -> str | None:
    ...
    try:
        ...
        entries, hub_truncated = dirwalk._scan(hub, probe_limits.PROBE_MAX_VISITS)
        matches = [
        ...
        if not matches:
            ...
            if hub_truncated:
                return (
                    f"<not found in the first {probe_limits.PROBE_MAX_VISITS} entries of "
        ...
        if best is None:
            if hub_truncated:
                return (
                    f"<no usable match in the first {probe_limits.PROBE_MAX_VISITS} entries "
        ...
        if hub_truncated:
            partial_scans.append(
                f"Hub listing truncated after {probe_limits.PROBE_MAX_VISITS} entries"
            )
        ...
        if snapshots.is_dir():
            ...
            # becomes "most recently used among the first probe_limits.PROBE_MAX_ENTRIES" —
            ...
            # part that did work.
            try:
                entries, snapshots_truncated = dirwalk._scan(
                    snapshots, probe_limits.PROBE_MAX_ENTRIES
                )
                if snapshots_truncated:
                    partial_scans.append(
                        "snapshot listing truncated after "
                        f"{probe_limits.PROBE_MAX_ENTRIES} entries"
                    )
                ...
                newest = _newest(
                    Path(e.path) for e in entries if dirwalk._is_dir_nofollow(e)
                )
...
def _resolve_snapshot_path(hub_root: Path, model_id: str) -> dict[str, Any]:
    ...
    if out["snapshots_dir_exists"]:
        try:
            ...
            entries, snapshots_truncated = dirwalk._scan(
                snapshots_dir, probe_limits.PROBE_MAX_ENTRIES
            )
            out["snapshot_subdirs"] = sorted(
                e.name for e in entries if dirwalk._is_dir_nofollow(e)
            )
        except OSError as e:
    ...
    if out["snapshot_subdirs"]:
        ...
        if snapshots_truncated:
            partial = (
                f"fallback selected from the first {probe_limits.PROBE_MAX_ENTRIES} entries; "
    ...
    if out["issue"] is None:
        if snapshots_truncated:
            out["issue"] = (
                f"no snapshot directory found in the first {probe_limits.PROBE_MAX_ENTRIES} "
...
def _snapshot_names(
    model_dir: Path, limit: int = probe_limits.PROBE_MAX_SNAPSHOTS
) -> tuple[list[str], bool]:
    """Snapshot names and whether the bounded listing omitted any entries.

    Two caps, because they bound different things: ``probe_limits.PROBE_MAX_ENTRIES`` bounds
    ...
    if not _paths.within(model_dir, snapshots):
        return [], False
    try:
        entries, truncated = dirwalk._scan(snapshots, probe_limits.PROBE_MAX_ENTRIES)
    except OSError:
        return [], False
    names = [e.name for e in entries if dirwalk._is_dir_nofollow(e)]
    return names[:limit], truncated or len(names) > limit


def find_model_dirs(
    root: Path,
    max_depth: int = probe_limits.PROBE_MAX_DEPTH,
    limit: int = probe_limits.PROBE_MAX_MATCHES,
    max_visits: int = probe_limits.PROBE_MAX_VISITS,
        ...
        try:
            with os.scandir(current) as scan:
                for entry in scan:
                    ...
                    if not dirwalk._is_dir_nofollow(entry):
                        continue
