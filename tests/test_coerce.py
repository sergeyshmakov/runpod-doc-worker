...
@pytest.mark.parametrize(
    "value", [["fast"], {"mode": "fast"}, {"fast"}, 3.5], ids=["list", "dict", "set", "float"]
)
def test_one_of_rejects_an_unhashable_or_non_string_choice(value: object) -> None:
    """`chosen not in allowed` raises a bare TypeError for an unhashable value.

    `allowed` is a set of strings, so a truthy non-string is invalid by definition
    -- but a list or dict made the membership test itself raise, which is a plain
    route out of the rejection envelope for any enum-shaped field a JSON body can
    reach.
    """
    with pytest.raises(ValueError) as caught:
        coerce.one_of("mode", value, frozenset({"fast", "accurate"}), "fast")

    assert str(caught.value).startswith(PREFIX)
    assert "mode must be one of" in str(caught.value)


def test_one_of_still_accepts_a_falsey_container_as_absent() -> None:
    """An empty list is falsey, so it means absent -- it must not hit the new guard."""
    assert coerce.one_of("mode", [], frozenset({"fast"}), "fast") == "fast"
